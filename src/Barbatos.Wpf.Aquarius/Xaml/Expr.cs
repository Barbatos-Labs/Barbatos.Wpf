...
/// <summary>
/// Evaluates a small comparison/arithmetic/logical expression over primitive-typed bound
/// properties - the closest Aquarius counterpart to writing a boolean expression directly
/// inline in a Vue template (<c>v-if="a + b >= c"</c>), which a plain WPF
/// <see cref="Binding"/> otherwise has no way to do (a binding path is just a property
/// path, not an expression language).
/// </summary>
/// <remarks>
/// <code>
...
/// lowercase <c>true</c>/<c>false</c></item>
/// </list>
///
...
///
/// <b>Deliberately out of scope</b> (primitives only, matching the request this was built
/// for): string concatenation via <c>+</c> (both consumers this was built for -
/// <see cref="If.Condition"/>/<see cref="Directives.ShowProperty"/> - are booleans; use
/// <c>StringFormat</c> or multiple <c>Run</c>s to build display text instead), the ternary
/// operator (<c>a ? b : c</c> - use <see cref="If.Else"/> instead), method calls, and
/// indexers. For anything beyond this grammar, a <see cref="Reactivity.Computed{T}"/> in
/// the ViewModel remains the right answer, same as Vue's own guidance to move non-trivial
/// template expressions into a computed property.
///
...
public class Expr : MarkupExtension
{
    ...
    public override object ProvideValue(IServiceProvider serviceProvider)
    {
        ...
        var multiBinding = new MultiBinding { Converter = new ExprConverter(ast) };
    ...
    private sealed class ExprConverter(ExprNode ast) : IMultiValueConverter
    {
        public object Convert(object[] values, Type targetType, object? parameter, CultureInfo culture)
        {
            ...
            // Directives.Show (both default true) - "fail open", not a special case.
